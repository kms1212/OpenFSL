...
error_t openfsl::fat32::FAT32::__insertCache(const std::string key,
...
std::pair<openfsl::fat32::CacheType, void*>
    openfsl::fat32::FAT32::__getCacheValue(
...
error_t openfsl::fat32::FAT32::__deleteCache(const std::string key) {
    cacheOpCountMutex.lock();
    cacheOpCount++;
    cacheOpCountMutex.unlock();
...
void openfsl::fat32::FAT32::__cacheMgrThreadRunner() {
    while (cacheEnabled) {
...
error_t openfsl::fat32::FAT32::enableCache(const size_t cacheMaxEntry_) {
    if (!cacheEnabled) {
        fsOperationMutex.lock();
        cacheEnabled = true;

        cacheMgrThread =
            std::thread(&openfsl::fat32::FAT32::__cacheMgrThreadRunner, this);
...
void openfsl::fat32::FAT32::disableCache() {
    if (cacheEnabled) {
        fsOperationMutex.lock();
        cacheEnabled = false;
