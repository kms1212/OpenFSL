...
error_t openfsl::fat32::FAT32::FAT32::__createEntry(
...
recalc:
    ...
    // Find free entry
    ClusterChain lcluster(this, currentCluster);
    size_t entryPlace = 0;
    ...
    fileEntry.fileCreateDate = __toFatDate(createFileInfo.fileCreateTime);
    fileEntry.fileCreateTime = __toFatTime(createFileInfo.fileCreateTime);

    fileEntry.fileAccessDate = __toFatDate(createFileInfo.fileAccessTime);

    fileEntry.fileModDate = __toFatDate(createFileInfo.fileModTime);
    fileEntry.fileModTime = __toFatTime(createFileInfo.fileModTime);

    if (requiredEntrySize > 1) {  // If file name is LFN
        // Initialize SFN entry to whitespace
        memset(fileEntry.fileName, ' ', 11);

        std::u16string filename16 = u8ToUCS2(createFileInfo.fileName);
